        ...
        <CardHeader className="bg-muted/40 pb-4 border-b border-border flex flex-row items-center justify-between">
          <div>
            ...
            <CardDescription className="text-xs text-muted-foreground mt-0.5">
              تصفح الهيكل المالي، تعديل بيانات الحسابات، حذف الحسابات غير المستخدمة، واستعراض كشوفات
              الحساب.
            </CardDescription>
          </div>
        </CardHeader>
        ...
        <CardContent className="p-0">
          {filteredAccounts.length === 0 ? (
            <div className="p-12 text-center space-y-3">
              ...
              <h3 className="font-black text-base text-foreground">لا توجد حسابات مطابقة للبحث</h3>
                      ...
                      >
                                            ...
                                            : acc.system_binding === "sales_revenue"
                                              ...
                                              : acc.system_binding === "operating_expenses"
                                                ...
                                                : acc.system_binding === "warehouse_main_value"
                                                  ? "قيمة المخزن الرئيسي"
                                                  : acc.system_binding === "warehouse_kitchen_value"
                                                    ...
                                                    : acc.system_binding ===
                                                      ...
                                                      : acc.system_binding ===
                                                        ...
                                                        : acc.system_binding.startsWith("treasury_")
                        ...
                        <td className="p-3.5 text-muted-foreground font-mono">
                          {acc.parent_code ? (
                            ...
                            <span className="text-muted-foreground/50 text-[10px]">حساب رئيسي</span>
                          )}
                        </td>
              ...
              >
                ...
                {erpState.treasuries.map((t) => (
                  <option key={t.id} value={`treasury_${t.id}`}>
                    🔗 ربط لحظي برصيد خزينة: {t.name_ar} ({t.currency})
                  </option>
                ))}
                ...
                <option value="warehouse_main_value">📦 ربط لحظي بقيمة بضاعة المخزن الرئيسي</option>
      ...
      <Dialog open={isOracleImportOpen} onOpenChange={setIsOracleImportOpen}>
        <DialogContent className="max-w-2xl dir-rtl text-right rounded-2xl" dir="rtl">
          <DialogHeader className="text-right border-b border-border pb-3">
            ...
            <DialogDescription className="text-xs text-muted-foreground mt-1">
              قم برفع ملفات الـ Excel الخاصة بمستويات الشجرة الأربعة وحركات القيود المالية وسندات
              الخزينة لنقلها بذكاء وربطها بالكامل للبدء في عام 2026.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <Input
                  type="file"
                  accept=".xlsx, .xls, .csv"
                  onChange={(e) => setOracleFiles({ ...oracleFiles, level1: e.target.files?.[0] })}
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  type="file"
                  accept=".xlsx, .xls, .csv"
                  onChange={(e) => setOracleFiles({ ...oracleFiles, level2: e.target.files?.[0] })}
                  className="text-xs h-10"
                />
              </div>
              <div className="space-y-1.5">
                ...
                <Input
                  type="file"
                  accept=".xlsx, .xls, .csv"
                  onChange={(e) => setOracleFiles({ ...oracleFiles, level3: e.target.files?.[0] })}
                  className="text-xs h-10"
                />
              </div>
              <div className="space-y-1.5">
                ...
                <Input
                  type="file"
                  accept=".xlsx, .xls, .csv"
                  onChange={(e) => setOracleFiles({ ...oracleFiles, level4: e.target.files?.[0] })}
                  className="text-xs h-10"
                />
              </div>
            </div>
            ...
            {importingOracle && (
              <div className="p-4 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 rounded-xl text-center space-y-2">
                ...
                <p className="text-xs font-bold text-amber-800 dark:text-amber-300">{importLog}</p>
              </div>
            )}
          </div>
