...
if (!code.includes('import { oracleAccounts }')) {
  code = code.replace(/import \{ erpStore \} from "\.\.\/shared\/services\/erpStore";/, 
    `import { erpStore } from "../shared/services/erpStore";\nimport { oracleAccounts } from "../shared/data/oracleAccounts";`);
}

// Add the Account code field UI after "الرصيد الافتتاحي" or "نوع الحساب والعملة"
const newField = `
